...
fn protocol_from_cli(
    ...
    let cipher_mode = match value(cli, cell, &["cipher-mode"]).unwrap_or("ctr") {
        "ctr" => shiguredo_srt::crypto::CipherMode::Ctr,
        "gcm" => shiguredo_srt::crypto::CipherMode::Gcm,
        other => {
            return Err(format!(
    ...
    Ok(ProtocolEnvelope {
        ...
        flow_window_packets: value_u32(
            ...
            shiguredo_srt::handshake::DEFAULT_FLOW_WINDOW as u64,
        )?,
        ...
        light_ack_interval_packets: value_u32(
            ...
            u64::from(shiguredo_srt::receiver::LIGHT_ACK_INTERVAL_PACKETS),
        )?,
...
fn input_from_bench_config(cfg: &crate::BenchConfig) -> CapacityInput {
    ...
    CapacityInput {
        ...
        protocol: ProtocolEnvelope {
            ...
            ack_interval: Duration::from_micros(cfg.ack_interval_micros),
            light_ack_interval_packets: cfg.light_ack_interval_packets,
            bond,
