...
//! 16x lower per-conn rate -- N=6 delivers perfectly, repeated 3x,
//! including bonded broadcast handoffs). It's a raw
//! per-packet-operation-overhead ceiling: owned-buffer io_uring
...
fn run_reuseport_multi(cfg: LossConfig, k: usize) {
    ...
    if handoffs > 0 {
        eprintln!("[bench-compio] pool receiver: {handoffs} bond handoffs");
    }
    agg.print(start);
    if !agg.any_connected {
