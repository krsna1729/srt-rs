...
//! `SrtConnection` + `srt_transport::advanced::driver::ManualTimerStore` + `listener.send_to`.
//!
...
fn executor_builder(cfg: &BenchConfig, index: usize) -> glommio::LocalExecutorBuilder {
    let placement = if cfg.pin {
        glommio::Placement::Fixed(index % srt_transport::advanced::platform::available_cpus())
    } else {
    ...
    while source.pending() > accepted {
        match driver.send_paced(payload, now).await {
            ...
            srt_transport::advanced::driver::PacedSendOutcome::Rejected(error) => {
                eprintln!("[bench-glommio] send_paced rejected (not a pacing refusal): {error}");
                source.refused();
                break;
            }
            srt_transport::advanced::driver::PacedSendOutcome::DriverError(error) => {
                eprintln!("[bench-glommio] send_paced driver error: {error}");
                stats.data_events += 1;
                accepted += 1;
                break;
            }
