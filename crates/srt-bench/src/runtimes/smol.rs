    ...
    while source.pending() > accepted {
        match driver.send_paced(payload, now).await {
            ...
            srt_transport::PacedSendOutcome::Rejected(error) => {
                eprintln!("[bench-smol] send_paced rejected (not a pacing refusal): {error}");
                source.refused();
                break;
            }
            srt_transport::PacedSendOutcome::DriverError(error) => {
                eprintln!("[bench-smol] send_paced driver error: {error}");
                stats.data_events += 1;
                accepted += 1;
                break;
            }
...
async fn sender_task(
    ...
    loop {
        ...
        wait_and_recv_sender(&mut driver, stats.connected, &mut buf, start, &source).await;
...
async fn flush_single_outbound(listener: &UdpSocket, outbound: &mut Vec<(SocketAddr, Vec<u8>)>) {
    for (peer, bytes) in outbound.drain(..) {
...
async fn run_single_acceptor(
    ...
    while Instant::now() < connect_deadline && routed < cfg.connections {
        admit_single_until_tick(&listener, &mut buf, &mut peers, &admission, start).await;
