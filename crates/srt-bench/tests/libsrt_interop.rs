...
// ---------------------------------------------------------------------------
// Compio `Owner` production path against real libsrt
// ---------------------------------------------------------------------------
//
// These traverse the exact production surfaces Restream's SRT egress uses: the
// production Compio runtime (`production_runtime_builder`), an `Owner` with
// its ONE shared caller UDP socket, the bounded `CallerPool`, a
// `LogicalCallerId`, and direct-final-buffer TX -- `Owner::connect` for a
