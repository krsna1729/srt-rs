...
| *(not upstream-tracked — found by inverse live encryption interop)* | High — rejection was effective but misclassified on the wire | KMRSP failure status used the generic big-endian SRT word encoder. libsrt copies `SRT_KM_STATE` into the KM payload before its handshake word swap, so its interoperable extension bytes are little-endian; the old `00 00 00 04` arrived as unknown state `0x04000000` and libsrt fell back to `NOSECRET` instead of recognizing `BADSECRET`. Encode/decode the four-byte KM failure payload explicitly as little-endian, assert the exact `04 00 00 00` wire bytes, and surface listener unwrap failure as terminal `HandshakeRejected` rather than a raw crypto error. Real `srt-live-transmit` callers with a mismatched passphrase now observe `BADSECRET`, complementing the existing Rust-caller → libsrt rejection test. `src/srt_handshake.rs`, `src/srt_connection.rs`; `test_km_error_response` and `libsrt_live_caller_wrong_passphrase_is_rejected_by_rust_listener`. |
...
| *(not upstream-tracked — missing capability, a transport correctness gap this repo found by measurement)* | High — a lost flight tail was stranded forever | The sender had no timeout of its own: `TimerId::Retransmit` was only ever armed by `process_retransmit` to continue an already-filled NAK-driven queue, so the retransmission path was reachable only through a NAK. A receiver can only NAK a gap that a *later* sequence number exposes, and a lost suffix of a flight exposes none — `sec_a` stayed zero, no loss was reported, and the payload was simply absent with nothing left to ask for it. Added `crate::sender_rto::SenderRto` and `TimerId::SenderRto`: a real elapsed-time timeout (`SRTT + 4*RTTVar + 2*COMM_SYN`, exponential backoff) armed when DATA is actually submitted to the transport, reset only on cumulative ACK progress, and expiring into exactly one probe of the newest submitted packet. The old continuation timer is now `TimerId::RetransmitContinue`, which carries no notion of loss or elapsed time. `src/sender_rto.rs`, `src/srt_sender.rs` (`note_data_submitted`, `SentPacket::submitted`, `newest_submitted`), `src/srt_connection.rs` (`poll_output_into` submission boundary, `handle_ack` progress, `handle_sender_rto_timeout`). Deterministic regressions fail without the trigger: `crates/srt-protocol/src/srt_connection.rs` (unit, five scenarios), `crates/srt-transport/tests/tail_recovery.rs` (through the real `ManualTimerStore`). |
| [0054](https://github.com/shiguredo/srt-rs/blob/develop/issues/0054-bug-fix-ackack-condition-too-loose.md) | (upstream: not labeled; spec-compliance) | `handle_ack` sent an ACKACK for any received ACK with `control_info.len() >= 16`, but the draft (draft-sharabayko-srt.md `#ctrl-pkt-ack`) only defines Full ACK (28-byte CIF) as acknowledged, not Small ACK (16-byte CIF). Originally changed to `>= 28`, following the draft alone. A later review checking the *pinned Haivision reference* (`899348d8`, `core.cpp`'s `CUDT::sendCtrl`/`processCtrlAck`) found that conclusion was itself incomplete: `CUDT::sendCtrl` emits a 4-, 16-, 24-, 28-, or 32-byte ACK CIF, while `CUDT::processCtrlAck` on the receive side accepts any 4-byte Light ACK or any non-Light ACK of at least 16 bytes -- materially wider than what it actually emits. Of the 16-byte Small form, pinned Haivision is shown emitting only the deployed numbered variant (Robotweax pins a regression for exactly this variant); the unnumbered form is the draft's own definition, which this crate also accepts. This crate deliberately accepts the bounded set of sizes pinned Haivision is actually shown to emit -- {4, 16, 24, 28, 32} -- matching Robotweax's bounded decoder, rather than mirroring Haivision's wider receive-side tolerance, and acknowledges every one of them that carries an ACK number with ACKACK, not only the 28-byte form. The condition is now `type_specific_info != 0` (equivalent to "carries an ACK number", which `validate_ack_shape` already enforces per accepted size), and `validate_control_information`/`handle_ack` accept and parse RTT/RTTVar/window feedback for the full `{4, 16, 24, 28, 32}` set rather than only `{4, 16, 28}`. `src/srt_connection.rs`. |
...
| Message reassembly and DROPREQ | Multi-packet message reassembly is implemented: `send_message` fragments payloads exceeding the negotiated MTU, and the receiver's `MessageAssembler` buffers fragments and delivers complete messages. `DROPREQ` is fully implemented: the sender emits it when TLPKTDROP drops a message, and the receiver processes it by purging the affected sequence range and any in-progress reassembly. Sender-side TLPKTDROP is message-aware (drops all fragments of a message together). Covered by `send_message_fragments_and_reassembles`, `dropreq_drops_receiver_message`, property tests (`fragmentation_reassembly_roundtrip`), and fuzz targets (`fuzz_message_reassembly`, `fuzz_connection_feed`). |
| Explicit live-only scope | `PEERERROR` and congestion warnings decode but do not affect the live-only API; FileCC is not exposed. Packet filter/FEC (`PACKET_FILTER` flag, `ExtensionType::Filter`) is recognized in the handshake parser but never advertised and not acted upon; a peer's filter extension is silently skipped. Rendezvous remains unsupported; caller/listener is the supported topology. Bonding balancing (group type 3) is parsed but rejected; Broadcast and Backup are the supported group modes. This is deliberate scope, not silent packet-decoding loss. |
