...
use shiguredo_srt::handshake::DEFAULT_FLOW_WINDOW;
use shiguredo_srt::receiver::{LossRange, NakPacket, ReceiverBuffer};
...
proptest! {
    ...
    fn configured_ack_interval_is_the_periodic_threshold(
        initial_seq in 0u32..0x7FFF_FFFFu32,
        interval_micros in shiguredo_srt::receiver::MIN_ACK_INTERVAL_MICROS
            ..=shiguredo_srt::receiver::MAX_ACK_INTERVAL_MICROS,
    ) {
        let start = Timestamp::from_micros(0);
        let mut buf = ReceiverBuffer::new(initial_seq, 120, start, 0);
        buf.set_ack_coalesce(interval_micros, shiguredo_srt::receiver::LIGHT_ACK_INTERVAL_PACKETS);
