...
* a rejected request is transactional exactly like a rejected `connect`. For
  the FIRST caller on an Owner (direct or bonded, one rule) the caller side is
  built as a candidate, the request is admitted into the candidate's pool, and
  only then is managed RX started and the side, receive mode and started-session
  flag committed, so a refusal at any earlier step leaves no side, no selected
  receive mode, no started session and no managed consumer or provided-buffer
  lease. A candidate side never owns a live receive, so it is dropped without
  the awaited cancellation a running consumer requires. A
