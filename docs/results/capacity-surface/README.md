...
Both are committed verbatim; neither is a rerun-until-green, and the second was
taken because the first showed a *source* stall, which is a host property rather
than a transport one -- reporting both is what makes that distinction visible.

```text
sweep A: F50-r8-K256-postfix.tsv    git_sha=206c5a2 git_dirty=false
sweep B: F50-r8-K256-postfix-b.tsv  git_sha=1cfdc45 git_dirty=false

rep  cadence     missed  f_drain   conserved  retx  sec_a  sec_b  fences  missing_final  fsub p50/p99  offer p99  C_SRT
A1   1.000000        0   0.0018%   yes          0      0      0      50              0    100/93.4ms      4.8ms   17.4
A2   1.000000        0   0.0019%   yes          0      0      0      50              0    100/105ms       3.8ms   17.0
A3   0.999342       30   0.0020%   yes          0      0      0      50     1500 (=30x50)   100/286ms       6.5ms   17.1
B1   0.998838       53   0.0019%   yes         33      0     33      50    2650 (=53x50)   100/271ms      14.8ms   18.4
B2   1.000000        0   0.0028%   yes          0      0      0      50              0    100/267ms      10.1ms   18.3
B3   0.999956        2   0.0019%   yes          0      0      0      50     100 (=2x50)   100/403ms      19.4ms   19.3

conserved = data_accepted + fence_accepted == rx_core_total, exactly, on every row
C_SRT     = window_cpu_ms * 1000 / data_accepted  (us per accepted payload)
```

What this establishes:

* **Conservation is exact, six times out of six.** The pre-fix defect this page
  spent its length chasing -- payloads accepted by the sender and never delivered,
  invisible to the receiver -- does not appear in any repetition. The fence
  payloads are the only difference between `accepted` and `rx_core_total`, and
  they are exactly the extra traffic the fence offered.
* **No unresolved loss anywhere**: `sec_a = 0` and `diag_duplicate_payloads = 0`
  in all six rows. One repetition (B1) retransmitted 33 packets, which the
  receiver counted as 33 duplicate *packets*; conservation and payload identity
  were unaffected. The artifact does not identify which stage dropped them, and
  this page does not guess: it is one bounded, fully recovered event inside the
  repetition with the largest host stall.
* **Stationarity holds by two orders of magnitude**: `f_drain` is 0.0018-0.0028 %
  against a declared 1 %, i.e. 50-140 wire datagrams of ~2.8 M. The pre-fix
  surface's other configurations sat at 33-76 %.
* **Cadence holds in five of six repetitions** (min 0.998838). The two rows that
  miss it are the same rows that miss the fence criterion, and for the same
  reason: the source loop stalled long enough that boundaries were dropped past
  its 64-tick catch-up cap.
* **`missing_final` is fully attributable where it is non-zero**: it equals
  `missed_source_ticks x established` exactly (1500 = 30 x 50, 2650 = 53 x 50,
  100 = 2 x 50), with `missing_scatter_peers = 50` -- every peer equally, i.e.
  whole ticks that were never offered, not per-peer loss. `data_offered` equals
  `generated_ticks x F` on every row, so the source never offered those payloads
  at all.

Under the frozen criterion list, that is **4 of 6 repetitions fully qualified and
6 of 6 on every transport-side criterion**: the only unmet clause is
`diag_missing_final == 0`, whose failures are source stalls. The honest summary is
therefore:

> F=50 x 8 Mbps x K=256 is **sustained** on this host -- every repetition keeps
> pace (f_drain <= 0.003 %), conserves exactly, reports no unresolved loss, and
> carries no Owner fault -- while the *source* fails to offer 0-53 of ~45 590
> boundaries in some repetitions. A capacity point that requires zero source
> stalls needs a host that does not stall a 60 s loop, which is the "quiet pinned
> host" this project has listed as a requirement all along.

Cost is unchanged: `C_SRT` is 17.0-19.3 us per accepted payload against the
pre-fix 17.1-18.0, and `r_window` is 1.219 as before. The recovery work this
release adds costs nothing measurable in a lossless run, which is the expected
result of a timeout that only fires when the peer has stopped making progress.

Real time remains **undeclared**. The offer itself is not punctual (p99 3.8-19.4
ms against the 5 ms budget the harness prints), and `first_submit_lateness` p99 is
93-403 ms: that tail is dominated by the same source catch-up, since a caught-up
tick carries a deadline already in the past. Neither number supports a real-time
claim, and neither is presented as one.
