...
| `first_submit_lateness_us_p50/p99/max` | source due instant to the point the datagram was handed to a TX lane, bucketed at 100 µs (p50/p99 are bucket upper bounds; `max` is exact) | this is the *transport's* delay after accepting a payload, not the source's punctuality and not end-to-end latency. Compare it against `offer_lateness_us_*`, which is sampled before `service()` and says nothing about the dataplane. Kernel/async completion latency is outside both |
